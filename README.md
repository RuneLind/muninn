...
| `PROMPT_SNAPSHOTS_CAPTURE_RETENTION_DAYS` | No | `90` | Days to keep CAPTURE prompt snapshots — the prompt the summary PASS was sent (not necessarily the prompt that produced the stored summary: the closing-takeaway check can rewrite the closer afterwards, and that call stores no snapshot). Read back by `GET /api/summaries/prompt?url=`, so it has to outlive both the 3-day chat window and the 7-day trace sweep. A value below 1 is refused with a warning and the default applies — `NOW() - 0 days` would delete the archive on the next tick; a value that is not an integer at all refuses the boot |
...
is reachable only by a hand-rolled POST for now. Both options endpoints also
answer `capture: { supported, reason? }` — the POST's own 503 pre-flight asked
ahead of time: the job needs to grant the model read access to its temporary
frame directory on every path, so a `SUMMARIZER_BOT` whose connector cannot do
that (anything but `claude-cli` / `claude-sdk`) runs no short-video capture,
`frames: false` included.
