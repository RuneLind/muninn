...
| `MUNINN_AUTH` | No | `off` | `off` is the default and is unchanged behaviour. `local` requires a session for HTTP requests: one pinned identity (`MUNINN_LOCAL_USER`) behind a shared secret (`MUNINN_LOCAL_TOKEN`, min 16 chars), presented on `X-Muninn-Token` / `Authorization: Bearer` / `?muninn_token=` and exchanged for an `HttpOnly` cookie. Also requires `MUNINN_ADMIN_IDENTS` and `MUNINN_ALLOWED_ORIGINS`; the process refuses to start without them. A **direct** loopback request bypasses auth by design, so a wrong secret is not a lockout. ⚠️ **This is a partial boundary**: routes that have an "own" version now derive the id from the session rather than the client, and `MUNINN_ALLOWED_ORIGINS` is enforced on side-effecting requests and on the CORS headers — but four admin-zone routes (`GET /api/messages/:userId`, `GET /api/users/:userId/overview`, `POST /api/users`, `PUT /chat/bot-preferences/:botName/default-user`) still accept a claimed id and nothing denies them until the zone model lands; the `/chat/ws` upgrade is not covered (no Hono middleware can see it); no resource is owned by id; collection routes still list every user's rows; nothing is denied by role; and the loopback bypass is blind to an L4 forward (`ssh -L`, `socat`, `tailscale serve --tcp`, a bare nginx `proxy_pass`) — behind one, every client is granted the pinned identity with no credential. An unauthenticated browser gets raw 401 JSON, not a login page. The four Chrome extensions in `extensions/` need their `chrome-extension://<id>` origin in `MUNINN_ALLOWED_ORIGINS` on an authenticating instance, or every capture and research POST is refused as cross-origin. It hardens a single-human instance against casual LAN/tailnet access; it does not make muninn multi-tenant. **Read `src/auth/CLAUDE.md` before exposing an instance.** |
