...
|---|---|
| No `users` table — never provisioned | Apply `db/init.sql` **and** `bun db/migrate.ts --baseline`, from a machine that has psql and can reach the database (this image ships neither psql nor the repo tooling) |
| Schema present, `schema_migrations` empty — provisioned but never baselined | Run `bun db/migrate.ts --baseline` (`bun run db:migrate:baseline`) |
