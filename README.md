...
| `MUNINN_AGENT_CWD` | No | `~/.muninn/agent-cwd` | Root for the working directories muninn's own `claude -p` spawns run in (one subdir per bot). Keeps watcher/scheduler agent sessions out of the repo's `~/.claude/projects/` folder and stops them loading the repo's CLAUDE.md — measured ~18k fewer prompt tokens per gate call. Only affects spawns with no explicit cwd; the email watcher and the extractors still run in `bots/<name>/`. |
