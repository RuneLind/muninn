...
| `MUNINN_AGENT_CWD` | No | `~/.muninn/agent-cwd` | Root for the working directories muninn's own `claude -p` spawns run in (one subdir per caller). Keeps those agent sessions out of the repo's own `~/.claude/projects/` folder and stops them loading the repo's CLAUDE.md — measured ~18k fewer prompt tokens per watcher gate call. Covers **every** `spawnHaiku` spawn plus the benchmark judge. A call that needs bot tools (today only the email watcher) asks for them by name — `--mcp-config` + `--settings` from `bots/<name>/` — instead of by running there; a call that needs the bot persona passes it as `--system-prompt`. Tool-less spawns additionally run `--strict-mcp-config`. Measured cold on an identical prompt: $0.070 → $0.023 per email-watcher call, $0.070 → $0.020 per tool-less call. Overrides that are relative, or that resolve inside the checkout (symlinks and filesystem case included), are refused with a warning. |
