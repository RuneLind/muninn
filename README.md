...
| `MUNINN_AGENT_CWD` | No | `~/.muninn/agent-cwd` | Root for the working directories muninn's own `claude -p` spawns run in (one subdir per bot). Keeps those agent sessions out of the repo's own `~/.claude/projects/` folder and stops them loading the repo's CLAUDE.md — measured ~18k fewer prompt tokens per watcher gate call. Affects only spawns with **no** explicit cwd (x/anthropic watchers, gardener cluster/map, interest profile); the email watcher and the extractors deliberately keep `bots/<name>/`, which is inside the repo and so still loads it. An override resolving inside the checkout is refused with a warning. |
