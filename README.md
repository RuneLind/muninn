...
| `MUNINN_AUTH` | No | `off` | `off` is the default and is unchanged behaviour. `local` requires a session for HTTP requests: one pinned identity (`MUNINN_LOCAL_USER`) behind a shared secret (`MUNINN_LOCAL_TOKEN`, min 16 chars), presented on `X-Muninn-Token` / `Authorization: Bearer` / `?muninn_token=` and exchanged for an `HttpOnly` cookie. Also requires `MUNINN_ADMIN_IDENTS` and `MUNINN_ALLOWED_ORIGINS`; the process refuses to start without them. A **direct** loopback request always bypasses auth by design. ⚠️ **This is a partial boundary**: the `/chat/ws` upgrade is not covered, routes still trust a client-supplied `userId`, and no resource is owned — so it hardens a single-human instance against casual LAN/tailnet access, it does not make muninn multi-tenant. See `src/auth/CLAUDE.md`. |
