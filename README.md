...
| `MUNINN_AGENT_CWD` | No | `~/.muninn/agent-cwd` | Root for the working directories muninn's own `claude -p` spawns run in (one subdir per caller). Keeps those agent sessions out of the repo's own `~/.claude/projects/` folder and stops them loading the repo's CLAUDE.md — measured ~18k fewer prompt tokens per watcher gate call. Covers `spawnHaiku` spawns with **no** explicit cwd (x/anthropic watchers, gardener cluster/map, interest profile) plus the benchmark judge unconditionally; the email watcher and the extractors deliberately keep `bots/<name>/`, which is inside the repo and so still loads it. Overrides that are relative, or that resolve inside the checkout (symlinks and filesystem case included), are refused with a warning. |
