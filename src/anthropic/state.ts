...
const jobs = new Map<string, AnthropicJob>();
...
export function getJob(jobId: string): AnthropicJob | undefined {
  return jobs.get(jobId);
}

export function getRecentJobs(limit = 20): AnthropicJob[] {
  return [...jobs.values()]
...
export function completeJob(jobId: string, summary: string, category: string): void {
  ...
  log.info("Anthropic job {jobId} completed, category: {category}", { jobId, category });
}
...
export function failJob(jobId: string, error: string): void {
  ...
  log.error("Anthropic job {jobId} failed: {error}", { jobId, error });
}
