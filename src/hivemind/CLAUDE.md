...
| `active-turn.ts` | **Two mechanisms.** (a) `runInActiveTurn(bot, threadId, fn)` / `currentActiveTurn()` — an `AsyncLocalStorage` binding `processMessage` wraps the CONNECTOR CALL in, so a connector can CAPTURE its turn at entry (`currentActiveTurn()`) and hand the binding down to the tool-result tail. Captured, never read at the point of use: `copilot-sdk` keeps ONE long-lived client for the process, and a handler dispatched from a read loop created inside the FIRST turn's scope reads that turn's store — measured. (b) The per-bot LIFO stack (`pushActiveTurn`/`peekActiveTurn`/`popActiveTurn`) — for callers with NO async link to the turn: muninn's own MCP servers are separate `Bun.serve` listeners answering the model's subprocess, so they inherit no store. **The stack races** (last push wins) and `peek` alone is a guess under concurrency, so `resolveActiveTurn(bot)` is the honest read: the binding first, then the stack while it holds only ONE DISTINCT thread, otherwise `{ threadId: null, ambiguous: true }`. Its only caller is `research/thread-citations.ts`, and only on the OUT-OF-BAND path — the in-band connector tail does not resolve anything, it carries the binding its connector captured at entry. The hivemind handlers below still `peek` unguarded — see the note under `delegate_task`. |
...
- **Run resolution is by ORIGIN THREAD, never the LLM-supplied `issueKey`.** It
  ...
  peeks) and `getDevRunByThreadId`. **This is the bare `peek`, deliberately** —
  unlike `research/thread-citations.ts`, which drops a write rather than guess
  under concurrency. The trade differs because the failure differs: a
  mis-attributed CITATION is a durable row in someone else's thread, while a
  mis-routed peer reply is an ephemeral message in the wrong conversation, and
  refusing to route it loses the reply outright. Re-decide this alongside the
  auth work if two people ever share one bot in earnest. Chat-started research has a synthetic
