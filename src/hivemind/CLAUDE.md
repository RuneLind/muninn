...
| `router.ts` | `HivemindRouter` — routes unsolicited inbound peer messages. Resolves peer-reply correlation first (see `correlation.ts`); falls back to `peer:<namespace>/<cwd-basename>` threads under the bot's default user. Autorespond looks up `getClient(botName, namespace)` so the outbound relay goes through the same WS the inbound came in on. `parsePeerThreadName` and `peerThreadNameFor` helpers expose the format. |
...
| `correlation.ts` | In-memory `(botName, peerId) → threadId` map with `PEER_CORRELATION_TTL_MS` TTL (1 hour, `config.ts`). Set on every outbound (MCP tools, chat `>`, autorespond reply); read by `router.route()` so inbound peer replies route back to the originating thread instead of the default `peer:<ns>/<name>` bucket. Last-write-wins per peer. In-memory only — restarts drop correlation and replies fall back to the default thread. |
...
13. **Peer-reply correlation is in-memory and races on concurrent turns.** `active-turn.ts` + `correlation.ts` route peer replies back to the originating thread (the one whose `processMessage` was on the stack when `ask_peer`/`send_to_peer`/the `>` outbound fired). Two concurrent turns on the *same* bot race — last `peek` wins, so the wrong thread can capture the reply. Rare in practice (typically one user per bot). Restart drops all correlation; replies fall back to `peer:<ns>/<name>`. Upgrade path: pass `?turn=<token>` on the connector's MCP URL and bind threadId per MCP session.
