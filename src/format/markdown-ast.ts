...
/**
 * The two shapes this pipeline turns into `<code>`: a fenced block, and an
 * inline backtick span.
 *
 * They are exported as SOURCES rather than as regexes because a `g`-flagged
 * regex carries `lastIndex` between calls, and because two callers building
 * their own from one string is the repo's way of saying "there is one rule
 * here" (`COMPONENT_TAG_SOURCE`, `CLAIM_VERDICT_SOURCE`, `HIGHLIGHT_TOKEN_CLASSES`).
 *
 * ⚠️ **These are deliberately NOT CommonMark.** `codeSpanRegions` below answers
 * "what will THIS renderer turn into code", and the only rule that cannot
 * disagree with the renderer is the renderer's own — measured: a `~~~`-fenced
 * block renders here as ordinary prose, so a CommonMark-accurate walk
 * (`fencedLineMask` in `wiki-integrate.ts`, which is right for the WRITE path)
 * would call it code and a wikilink inside it would silently stop being a link.
 * If the fence grammar here ever grows, both consumers follow for free.
 */
export const CODE_FENCE_SOURCE = "```(\\w*)\\n([\\s\\S]*?)```";
/** Applied per LINE, exactly as `renderInline` applies it. */
export const INLINE_CODE_SOURCE = "`([^`]+)`";

const CODE_BLOCK_RE = new RegExp(CODE_FENCE_SOURCE, "g");
const CODE_PLACEHOLDER_RE = /^\x00CB(\d+)\x00$/;

/** A half-open `[start, end)` span of the source that renders as `<code>`. */
export interface CodeSpanRegion {
  start: number;
  end: number;
}

/**
 * Every region of `text` that {@link parseBlocks} + `renderInline` will render
 * as code — fenced blocks first, then inline spans on what is left.
 *
 * **Why this exists.** `src/wiki/render.ts` and `src/wiki/ask-render.ts` park a
 * `[[wikilink]]` / an `[n]` citation as a `\x00`-delimited sentinel BEFORE
 * calling `formatWebHtml` and restore it by regex over the RENDERED HTML. That
 * pass had no notion of code, so a wikilink written inside a fence or inside
 * backticks was substituted like any other — and a RESOLVABLE one became a live
 * `<a>` inside `<pre><code>` with the brackets gone from the fence's own text.
 * `code.textContent` is what the Copy button hands over, what `wiki-mermaid.ts`
 * reads to build a diagram and what the fact-check evidence card clones, so that
 * is altered source, not a display bug. Measured 2026-08-30 over mimir + the
 * jarvis wiki: 1495 wikilinks sit inside code across 57 pages, 522 of them in
 * INLINE spans — which is why this covers both shapes and not just fences.
 *
 * The order mirrors the renderer exactly: `parseBlocks` extracts fences from the
 * whole document first (so a fence inside a `<Callout>` or a `<CodeTabs>` is
 * found here too), and only then does `renderInline` scan each remaining LINE
 * for backtick pairs. Fenced regions are therefore blanked — same length,
 * newlines preserved, so every offset stays valid on the original — before the
 * inline pass runs, which is also what keeps a fence's own backticks from
 * pairing with prose ones.
 *
 * Two divergences from the renderer are known, both requiring a fence delimiter
 * to start or end mid-line, and both harmless (a wikilink renders literal rather
 * than as a link; nothing is corrupted and no sentinel escapes): the renderer
 * joins the text before an opener to the text after its closer onto one line,
 * where this keeps them apart; and a caller's parking may remove a backtick that
 * sits inside a wikilink TARGET, which this scan — running on the unparked
 * body — still sees.
 *
 * Callers test the START offset of a match and skip parking when it is inside a
 * region, which leaves today's matching behaviour untouched: a construct that
 * straddles a code boundary is parked exactly as before.
 */
export function codeSpanRegions(text: string): CodeSpanRegion[] {
  const fences: CodeSpanRegion[] = [];
  for (const m of text.matchAll(CODE_BLOCK_RE)) {
    fences.push({ start: m.index, end: m.index + m[0].length });
  }
  // Blank each fence — same length, newlines preserved, so every offset below is
  // still valid on the original — before the inline pass, mirroring `parseBlocks`
  // swapping the fence for a backtick-free placeholder ahead of `renderInline`.
  // Skipped entirely on a page with no fence, which is MOST of them and is the
  // one allocation here that scales with the whole document rather than with its
  // code. Measured warm over mimir + the jarvis wiki (1561 pages, 15.9 MB):
  // 62% carry no fence, the whole corpus scans in 106 ms (~68 us/page), and the
  // largest page in either wiki — 958 KB — costs 2.2 ms of its 35.9 ms render.
  let scanned = text;
  if (fences.length > 0) {
    const chars = text.split("");
    for (const f of fences) {
      for (let i = f.start; i < f.end; i++) if (chars[i] !== "\n") chars[i] = " ";
    }
    scanned = chars.join("");
  }

  const regions = fences;
  const inlineRe = new RegExp(INLINE_CODE_SOURCE, "g");
  let offset = 0;
  for (const line of scanned.split("\n")) {
    for (const m of line.matchAll(inlineRe)) {
      regions.push({ start: offset + m.index, end: offset + m.index + m[0].length });
    }
    offset += line.length + 1;
  }
  // Fences are found in document order and the inline pass walks lines in order,
  // but the two lists interleave — sorted so `inCodeSpan` can binary-search.
  return regions.sort((a, b) => a.start - b.start);
}

/** Is `index` inside one of `regions`? Binary search; `regions` must be sorted,
 *  which is what {@link codeSpanRegions} returns. */
export function inCodeSpan(regions: readonly CodeSpanRegion[], index: number): boolean {
  let lo = 0;
