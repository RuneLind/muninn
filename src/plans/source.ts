...
// The paths and the two enums live in `constants.ts` (which imports nothing), so
// a caller can have them without this module's registry → `src/db/` import cost.
// Re-exported here because this is where they were, and where they read.
export {
  PLANS_DIR,
  QUEUE_REL_PATH,
  PLAN_STATUSES,
...
/** How many leading lines may precede an opening `---` and still count as a
 *  fence someone MEANT to write. Three covers the shapes seen in the corpus (a
 *  leading blank line, a stray title, an editor's trailing newline artefact)
 *  while staying far above any body-level fenced example. */
const FENCE_HEAD_LINES = 3;

/** Whether the file's opening region looks like an attempted frontmatter fence.
 *  A `---` further down is a horizontal rule or a fenced example, not a fence. */
function attemptedFence(normalized: string): boolean {
  return normalized
    .split("\n", FENCE_HEAD_LINES)
    .some((line) => line.trim() === "---");
}
...
export function planRecordFromContent(
  ...
  if (!rawStatus) {
    ...
    //
    // "Somewhere" is not enough on its own, though: a lifecycle doc that SHOWS
    // frontmatter inside a ```yaml block carries the key in its body and has no
    // fence at all — warning on it puts a permanent line in the aggregated warn
    // for a file behaving correctly. So the file must also have ATTEMPTED a
    // fence: a `---` in the opening line region (line 1, or within the first
    // three lines, which is where a leading blank line or a stray title puts it).
    if (/^plan_status:/m.test(normalized) && attemptedFence(normalized)) {
      warnings.push(
