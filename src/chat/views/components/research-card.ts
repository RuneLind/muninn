...
export function researchCardScript(): string {
  ...
  function buildStartBuildingPrompt(planPath, specPath, roles) {
    ...
    return pathLines + goal +
      ...
      'Candidate BUILD agents are peers whose cwd-basename matches an actual melosys code repo (e.g. melosys-api, melosys-web, melosys-eessi, melosys-trygdeavgift-beregning, faktureringskomponenten). ' +
      (wantTest ? 'The candidate TEST agent is the peer whose cwd-basename is melosys-e2e-tests. ' : '') +
      'Ignore generic / multi-repo / non-repo peers whose cwd-basename does NOT match a real repo (e.g. a "melosys-claude-code" or "claude" peer), muninn bots (cwd under .../muninn/bots/), and other infra peers — they do NOT qualify.\\n\\n' +
      'Read the work plan to identify EVERY repo it touches. ONE BUILD AGENT PER REPO: for each repo the plan touches, recommend the online peer whose cwd-basename equals that repo name — backend work goes to the melosys-api peer, frontend work goes to the melosys-web peer, never one peer doing work across repos. If the plan touches two repos, that is TWO build recommendations (and on Confirm, TWO delegate_task calls), not one. ' + (wantTest ? 'Also recommend ONE TEST agent in melosys-e2e-tests. ' : '') +
      'Present, grouped by repo:\\n' +
      '- For EACH touched repo: Recommended BUILD agent (peer id) — repo + branch + one-line reason (which slice of the plan it owns)\\n' +
      ...
      'When I confirm, use the delegate_task tool (NOT send_to_peer) ONCE PER recommended build agent (one per repo touched)' + (wantTest ? ' plus once for the test agent' : '') + ' so each handoff is tracked under this dev run. Each call takes the params to (the peer id), role, and a message SCOPED to that peer\\'s repo: ' +
      'delegate_task(to: <build peer for repo X>, role: "build", message: <point it at the work plan, name the specific files/changes in repo X, and instruct it to ' + handoffReviewInstruction() + '>)' +
  ...
  function confirmHandoffPrompt(roles, planPath, specPath) {
    roles = roles || ['build'];
    var wantTest = roles.indexOf('test') !== -1;
    return 'Confirmed — proceed with the handoff now using the delegate_task tool (NOT send_to_peer) so each handoff is tracked under this dev run. ONE call per recommended BUILD agent (one per repo touched)' + (wantTest ? ' plus one call for the test agent' : '') + ', NEVER one peer doing work across repos:\\n' +
      '- For EACH recommended build agent: delegate_task(to: <that build peer>, role: "build", message: ...) — the message points it at the work plan' + (planPath ? ' (' + planPath + ')' : '') + ', names the specific files/changes for THAT peer\\'s repo, and instructs it to ' + handoffReviewInstruction() + '\\n' +
