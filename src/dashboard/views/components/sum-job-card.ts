...
export function sumJobCardScript(): string {
  return `
    ...
    var VIMEO_SENTENCES = {
      ...
      whisper_unavailable: 'This video has no caption track, and this machine cannot transcribe it (whisper-cli and a multilingual model are needed — see VIMEO_WHISPER_MODEL_PATH)',
      transcription_failed: 'This video has no caption track, and transcribing its audio failed',
      no_speech: 'This video has no caption track, and no speech was found in its audio',
      duplicate: 'Already captured',
    ...
    function connectSSE(jobId, source) {
      ...
      eventSource = sseClient(sourceApiBase() + '/stream/' + jobId, {
        ...
        complete: function(e) {
          // A vertical whose STORED summary differs from what streamed ships it
          // on the complete event — every vertical now, since the closing-takeaway
          // check can rewrite the closer after it streamed (TikTok, X-article and
          // YouTube had rewrites of their own before). An empty payload keeps the
          // accumulated text.
          if (e && e.data) {
            try {
