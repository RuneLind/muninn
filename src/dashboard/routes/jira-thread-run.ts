/**
 * Running a Jira draft AS A TURN in a chat thread — the only way a draft is
 * written.
 *
 * The model sees the conversation, because the conversation is where the task was
 * actually worked out: the melosys chat retrieves over the same three collections
 * through its own knowledge tools, and condensing that thread back into pasted
 * notes threw away both the retrieval and every adjustment the person made. (The
 * notes path that did exactly that — a fenced one-shot over a server-assembled
 * prompt — was deleted; only its tail survives, `finalizeJiraDraft`.)
 *
 ...
 * Each click is another turn, on its own row, naming its own message.
 *
 * **The row is the record on every terminal path**, which is what makes the
 * card's poller the single source of truth.
 */
...
export async function runJiraThreadDraft(
  ...
  try {
    ...
    //
    // Best-effort: a draft that generated fine must not be failed by a
    ...
    // handful. The slice is meaningful because `seedThreadCitations` orders
    // conversation-USED sources first — so the shallow slice is exactly the set
    // the discussion actually leaned on.
    //
    // …and then narrowed AGAIN, to what the draft actually NAMED. This prompt
    // carries no citation block at all — the conversation is the context — so
    // "the citations the model was given" is not a set this path has, and
 ...
 *
 * Module-private, and it has exactly ONE caller — the run above.
 *
