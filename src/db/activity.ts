...
export async function saveActivity(params: SaveActivityParams): Promise<void> {
  const sql = getDb();
  await sql`
    INSERT INTO activity_log (type, user_id, username, text, duration_ms, cost_usd)
    VALUES (${params.type}, ${params.userId ?? null}, ${params.username ?? null}, ${params.text}, ${params.durationMs ?? null}, ${params.costUsd ?? null})
  `;
}
...
export async function getRecentActivity(limit = 50): Promise<ActivityEvent[]> {
  ...
  const rows = await sql`
    SELECT id, type, user_id, username, text, duration_ms, cost_usd, created_at
    FROM activity_log
