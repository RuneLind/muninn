...
Two properties of that measure are load-bearing and non-obvious. It compares through the persist-time containment transforms (`containDraftBodyLinks` rewrites `[[Foo]]` to `**Foo**` INSIDE prose, so a raw comparison reads a delinked sentence as deleted). And it measures SURVIVAL, not sameness: prose the reviser adds is invisible to it, deliberately, because every restored block is introduced by a one-line lead the prompt asks for.
