...
Nothing in that path writes to a wiki — it persists proposals a human approves. The script refuses to persist a revision that scores worse or recovers nothing (`judgeBackfill`, `source-backfill.ts`), so no reviewer is handed a diff that makes the page worse.
