...
`source-drafter.ts` takes the drafted page from the one-shot's **return text**, so its model call is fenced against the file-writing tools (`FENCED_EXCLUDED_TOOLS` in `src/core/fenced-one-shot.ts`, re-exported as `DRAFTER_EXCLUDED_TOOLS`; the synthesis drafter above and the fact-check integrate proposer share the same seam) — without that fence the model can satisfy the prompt by writing the `.mdx` to disk and replying "File created successfully…", which parses to no frontmatter title and silently drops the draft (3 lost in one week, 2026-07-28). The fence is three layers — a built-in allow-list (`FENCED_ALLOWED_TOOLS`: Read/Glob/Grep), no MCP servers (`mcpDisabled`), and the deny-list — because the deny-list alone drifted: Claude Code 2.1.281 shipped `Workflow`, `Monitor`, `SendMessage` and `EnterWorktree` past it. An EMPTY `allowedTools` still means the full surface; the allow-list binds only when non-empty.
