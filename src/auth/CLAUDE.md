...
The shared secret, min 16 chars (`openssl rand -hex 24`). Presented on `X-Muninn-Token`, `Authorization: Bearer` or `?muninn_token=` and exchanged for a signed `muninn_session` cookie (`HttpOnly`, `SameSite=Lax`, 7 days); the cookie never carries the secret, and rotating this invalidates every session.

## `MUNINN_LOCAL_USER` — the full entry from the root `CLAUDE.md` env table

The single `users.id` every local session acts as. Optional `MUNINN_LOCAL_NAME` is its display name. Resolves to role `MUNINN_LOCAL_ROLE` (default `user`; `admin` only for a credential-channel request — a browser on the host stays `user`), see the row below and `src/auth/CLAUDE.md`.

## `MUNINN_LOCAL_ROLE` — the full entry from the root `CLAUDE.md` env table
