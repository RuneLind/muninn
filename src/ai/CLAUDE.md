...
| `haiku-direct.ts` | Direct `@anthropic-ai/sdk` adapter — `callHaikuDirect` (SDK) + `callHaikuWithFallback` (router that prefers SDK when `HAIKU_DIRECT_ENABLED=1` and auth is present, falls back to `spawnHaiku` on any error). Used by `knowledge-decomposer.ts` to skip CLI cold-start on the `research_knowledge` hot path. ~6× speedup vs CLI on the decomposer prompt. |
