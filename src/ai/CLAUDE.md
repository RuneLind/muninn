...
| `knowledge-api-client.ts` | Knowledge API fetch helper (`fetchKnowledgeApi`, its text siblings `fetchKnowledgeApiText` and `fetchKnowledgeApiSourceText` — the latter `null` unless the body is `text/*` — and the `knowledgeApiHandler` Hono wrapper with an optional `transform`; `KnowledgeApiError`) with AbortController timeout. Consumed by `research-knowledge.ts` and several dashboard routes. Also moved out of `dashboard/routes/` in the 2026-06 layering cleanup. |
