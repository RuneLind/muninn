...
|---|---|---|
| `connectors/claude-cli.ts` | `claude-cli` | Spawns `claude` CLI with `--output-format stream-json --verbose`. Reads NDJSON. CWD = bot dir for MCP/settings discovery. Maps `allowedTools`→`--tools` (built-in allow-list) and `excludedTools`→`--disallowedTools`; `mcpDisabled`→`--strict-mcp-config` with no `--mcp-config`, which drops the user-global servers too. |
