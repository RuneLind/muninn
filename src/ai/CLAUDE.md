...
| `vertex-access.ts` | Reaching **Vertex AI** over plain HTTP, for the connectors that speak HTTP themselves (the Agent SDK reads its own env names — see `resolveVertexConfig`). Three things: `isVertexEndpoint` (host-based, so a Vertex `baseUrl` needs no new config field — a static `OPENAI_API_KEY` against a Vertex host is a guaranteed 401, measured, so nothing else could have been meant), `assertVertexEndpointAllowed` (refuses the `global` region through the THREE doors the OpenAI-compatible URL offers — the region-less host, which IS the global endpoint; a `global-` host prefix; and `/locations/global/` in the resource path, which Vertex takes as authoritative however European the hostname reads. That last one is compared **decoded**: `URL.pathname` keeps percent-escapes while Google's frontend decodes them, so `%67lobal` and `global` address the same resource — measured, both answer 200 — and a raw compare refused one spelling while minting a bearer token for the other) and `VertexTokenProvider` (a cached, single-flighted ADC access token: metadata server first with a 700 ms budget, `gcloud auth application-default print-access-token` second at ~700 ms a call). gcloud's reported expiry is deliberately NOT parsed — `--format=json`, the obvious way to ask, prints the OAuth client secret and the refresh token — so a gcloud token is trusted for a conservative 30 min and the 401 retry below covers a wrong guess at the cost of one round trip. |
