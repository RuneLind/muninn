...
| `knowledge-api-client.ts` | Knowledge API fetch helper (`fetchKnowledgeApi`, its text sibling `fetchKnowledgeApiText`, and the `knowledgeApiHandler` Hono wrapper with an optional `transform`; `KnowledgeApiError`) with AbortController timeout. Consumed by `research-knowledge.ts` and several dashboard routes. Also moved out of `dashboard/routes/` in the 2026-06 layering cleanup. |
...
| `haiku-direct.ts` | Haiku router with four backends — `cli` (Claude CLI subprocess), `anthropic` (`@anthropic-ai/sdk`), `copilot` (`@github/copilot-sdk`, reuses the shared CopilotClient singleton) and `vertex` (`haiku-vertex.ts`, below). `callHaikuWithFallback` picks the backend via `resolveBackend()` (explicit `opts.backend` → `HAIKU_BACKEND` env → legacy `HAIKU_DIRECT_ENABLED=1` alias for anthropic → per-bot default from `opts.connector` → `cli` floor), then falls back to the CLI on any error — **except on `MUNINN_PROFILE=nais`, where there is no CLI**: the refusal lives in `spawnHaiku` (`haiku-cli-unavailable.ts`, one door, so the watchers that call it directly are covered too) and the router only hands down which backend it tried and why, so the thrown `HaikuCliUnavailableError` names the missing credential. Used by `knowledge-decomposer.ts` (research_knowledge hot path) and `haiku-extraction.ts` (memory / goal / schedule extractors). ~6× speedup vs CLI on the decomposer prompt. |
| `haiku-vertex.ts` | The router's **Vertex AI** backend — the short async calls sent to the same OpenAI-compatible endpoint the `openai-compat` chat turn uses, so the `research_knowledge` decomposer stops leaving for a different provider than the answer it feeds. Not a second credential path: `createAuthorizer` picks the Google-token shape off the host (and refuses `global`) and `requestWithRefresh` is the one implementation of the expired-token retry — this module owns only the one-shot request, which is non-streaming like `callHaikuDirect`. Two things it decides: `vertexOpenAiBaseUrl` builds a REGION as a host prefix and a MULTI-REGION as `aiplatform.<mr>.rep.googleapis.com` (measured — `eu-aiplatform.googleapis.com` is not a name), and `resolveVertexHaikuTarget` reads the same project/region names as `resolveVertexConfig` but deliberately NOT `CLAUDE_CODE_USE_VERTEX` (the Agent SDK's switch; this backend is selected by `haikuBackend`), returning a REASON PHRASE (not a bare variable name — both call sites render it as-is, since appending "is not set" misreported a set-but-malformed region) so the router can skip the attempt instead of spending a failing call. There is no connector-derived default for it: `openai-compat` is equally the local-Ollama shape, and the router is not given the bot's `baseUrl`. Anthropic-on-Vertex (`@anthropic-ai/vertex-sdk`) is deliberately not built — measured 2026-08-28, no Anthropic model is callable in the target region. |
...
| `vertex-access.ts` | Reaching **Vertex AI** over plain HTTP, for the connectors that speak HTTP themselves (the Agent SDK reads its own env names — see `resolveVertexConfig`). Three things: `isVertexEndpoint` (host-based, so a Vertex `baseUrl` needs no new config field — a static `OPENAI_API_KEY` against a Vertex host is a guaranteed 401, measured, so nothing else could have been meant), `assertVertexEndpointAllowed` (refuses the `global` region through the THREE doors the OpenAI-compatible URL offers — the region-less host, which IS the global endpoint; a `global-` host prefix; and `/locations/global/` in the resource path. **The HOST is what binds, and that is what was measured**: Google's refusal for a blocked region names the endpoint host, the region-less host is a live route to the global endpoint (`aiplatform.googleapis.com.` answers 301 to it — which is why `hostOf` strips the trailing dot, or `isVertexEndpoint` would say "not Vertex" and hand the request to the static-key path unguarded), and against `endpoints/openapi` a regional host answers 200 for `locations/global`, `locations/%67lobal` AND `locations/nosuchregion` alike — the segment is not validated there. The `global-` prefix host 404s and is refused as hygiene, not because it routes. An earlier version of this line cited the first two as proof the path routes; the third is the control that refutes it. The path door is kept anyway — a URL naming `global` is confused at best, and other Vertex path shapes do resolve the location from the path — and its segment is normalized three ways, percent-decoded, lowercased and stripped of trailing dots, because a door that refuses `global` and admits `%67lobal` or `global.` is not a door) and `VertexTokenProvider` (a cached, single-flighted ADC access token: metadata server first with a 700 ms budget, `gcloud auth application-default print-access-token` second at ~700 ms a call). gcloud's reported expiry is deliberately NOT parsed — `--format=json`, the obvious way to ask, prints the OAuth client secret and the refresh token — so a gcloud token is trusted for a conservative 30 min and the 401 retry below covers a wrong guess at the cost of one round trip. |
...
|---|---|---|
| `connectors/claude-cli.ts` | `claude-cli` | Spawns `claude` CLI with `--output-format stream-json --verbose`. Reads NDJSON. CWD = bot dir for MCP/settings discovery. Maps `excludedTools`→`--disallowedTools`; `toolsDisabled`→`--tools ""` + `--strict-mcp-config` with no `--mcp-config`, which drops the user-global servers too. |
...
| `haiku-vertex.test.ts` | The Vertex Haiku backend: regional vs multi-region URL building, target resolution (name precedence, the missing-variable answer, whitespace-is-unset), the request body/headers, the 401-retried-once / 403-not shape and the `global` refusal before any request. Its own `bun test` link — it stubs the DB with `mock.module` |
...
| `connectors/openai-compat-request.test.ts` | The WIRING: a real `executePrompt` over a real socket against a `Bun.serve` fake — static-key headers, the empty-response retry, a 401 retried once with a fresh token and no replayed `text_delta`, 403 not retried, a `global` baseUrl failing before any request. Its own `bun test` link: it swaps `globalThis.fetch` and drives the process-wide token provider |
