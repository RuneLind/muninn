...
**Cost on the ring/Recent (`costUsd`).** `completeRequest` accepts an optional `costUsd` and stamps it onto the `AgentRun` (so the ring-sourced `/agents` Recent renders a `$` figure for capture/research/digest/scheduled_task). Producers thread it from their `ClaudeExecResult.costUsd`/`HaikuResult.costUsd`; the gardener drain and goal reminders complete with no meta, so their cost is unknown and the column dashes. **`undefined` ≠ `0`**: direct-SDK Haiku backends leave `costUsd` undefined (⇒ dash), but subscription chat connectors (copilot-sdk / openai-compat) return an explicit `0` (⇒ `$0.00`, truthful "no incremental cost"). Chat + watcher Recent are trace-sourced, so their cost is read off the span (child `claude` span for chat, the childless watcher span's OWN `costUsd` attr for watchers) via the same `COALESCE` as `model` in `getRecentAgentTraces`; the client renderer is `fmtCost` (`views/components/helpers.ts`).
...
- **`capture`** — `src/summaries/job-store.ts` `createJobStore` (ONE factory hook covers all four verticals: youtube / x-article / tiktok / anthropic). `createJob`→`startRequest` (name `"<label>: <title-or-url>"`), `completeJob`/`failJob`/TTL-sweep→`completeRequest`. The run starts at **route level**, before the summarizer bot is resolved and before the model call, so it opens with an empty botName and no model/tokens — `attachRun(jobId, meta)` late-binds them from `runCaptureOneShot` (`src/summaries/summarizer-shared.ts`): bot/connector/model are pushed onto the LIVE run (`setBotName`/`setConnectorLabel`/`setModel`), while traceId + token totals are parked and handed to `completeRequest` at the terminal transition — that snapshot is what the completed-runs ring (i.e. `/agents` Recent) renders. An `attachRun` for an unknown or already-settled job is a silent no-op.
- **`research`** — `src/research/ask.ts` `streamResearchAnswer` (covers both `/research` and wiki Ask). Phases mirror searching→synthesizing; completed in a `finally` (done + error/abort), now carrying the synthesis tokens (previously dropped). It also **roots the trace** (`research_ask`) and passes `tracer.context` into `researchKnowledge`, so retrieval nests as a child instead of owning a separate, already-finished trace — and the synthesis `claude` call, formerly spanless, is its sibling. One trace per question.
- **`digest`** — `src/wiki/digest.ts` `generateWikiDigest` (the "What's new" card, fired on every `/wiki` open that misses the cache). Registered + traced (`wiki_digest` root + a `claude` span with model/tokens/cost) only once the cheap local I/O has passed its early returns, so a wiki with no `log.md` produces no run and no empty trace. Recent is ring-sourced (`RING_RECENT_KINDS`).
- **`profile`** — `src/profile/generator.ts` `refreshInterestProfile` (the weekly interest distillation; a declared kind with NO producer until this was wired). Registered + traced (`interest_profile` root + a `haiku` span) only AFTER the no-goals-and-no-memories skip, which fires on every tick for an empty user. Settled in a `finally` — load-bearing, because the empty-output and shape-gate paths return early without throwing. Recent comes from its `haiku_usage` row (`interest_profile`, via `getRecentExtractorUsage`), NOT the ring — so `profile` is deliberately absent from `RING_RECENT_KINDS`.
