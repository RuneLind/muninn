...
const jobs = new Map<string, XArticleJob>();
...
export function getJob(jobId: string): XArticleJob | undefined {
  return jobs.get(jobId);
}

export function getRecentJobs(limit = 20): XArticleJob[] {
  return [...jobs.values()]
...
export function completeJob(jobId: string, summary: string, category: string): void {
  ...
  log.info("X article job {jobId} completed, category: {category}", { jobId, category });
}
...
export function failJob(jobId: string, error: string): void {
  ...
  log.error("X article job {jobId} failed: {error}", { jobId, error });
}
