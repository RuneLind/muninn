...
info line (`-O after_move:<YTDLP_INFO_TEMPLATE>`) arrives *after* the download it
would have to bound. So the frames path opens with one
`yt-dlp -O <YTDLP_INFO_TEMPLATE> --skip-download --no-playlist` probe
(`probeVideoInfo`, ~3 s) — five fields, never the full info JSON, which measured
11.7 MB on one video and overflowed `runProc`'s 8 MB cap into a `Broken pipe`, and its `duration` drives all four things that
...
## The `YouTube` entry from the root `CLAUDE.md` module table

Transcript fetch + summarization (backs `youtube-routes.ts` + Chrome extension). Transcript comes from **huginn** (`GET {knowledgeApiUrl}/api/youtube/transcript/<id>`) and the finished summary is ingested back into a huginn collection — but the summarization itself runs **in muninn**, on `SUMMARIZER_BOT`'s connector. **It no longer always leaves the video alone: `frames: true` on the route body** (the extension popup's Slides tick, off by default, remembered in `chrome.storage.sync`) **pulls a video-only ≤720p rendition through yt-dlp and quotes its slides inline**, through the same source-neutral seam Vimeo uses (`src/summaries/frames.ts` — cadence, served root, prompt section, id gate, `keepReferencedFrames`; `src/youtube/frames.ts` is the vertical's own PURE half). **How those slides are CHOSEN is this vertical's alone, and it is a DENSE SCAN plus a SELECTION model call** (`src/youtube/scan.ts` pure, `scan-run.ts` the ffmpeg half; `YOUTUBE_FRAME_SCAN=cadence` restores the old sampler): one ffmpeg decode over the whole file at 1 frame per `YOUTUBE_SCAN_INTERVAL_SEC` (**5**, `fps=1/N:round=up:start_time=0` — the default `near` rounding emits the LAST frame of each bucket, so every sample carried the picture from half an interval later than its own name: 59 of 110 candidates disagreed with an `-ss <t>` re-grab of their own second, 0 of 111 after) emitting 320 px thumbnails AND a 32×18 grayscale signature per sample off the same decode, a per-BLOCK dedup against the previous KEPT sample (8×6 blocks, ≥12 gray levels, threshold 0.15 — measured on the reference video: 354 samples → 111 candidates with a frame inside all three chart windows, where a whole-frame mean cannot tell an inset presenter's head from a page turn and `select='gt(scene,0.25)'` misses two of the three charts outright), a cap at `YOUTUBE_CANDIDATE_CAP` (**120**) that reserves a third of itself for anchors evenly spaced over the video's DURATION — not over the candidate INDEX, where the anchors follow the candidates rather than the video and a half-hour tail behind a busy opening kept 4 of its 30 — BEFORE ranking by change, `CONTACT_SHEET` (**4×3 at 320 px**, the named benchmark variable) tiles under `mediaDir/select`, fed to the selection pass as its ONLY `extraDirs` so that neither the summary call nor a cadence fallback is handed them, and a full-height re-grab of exactly what that pass named. **Every cell carries its own burned-in label** (`#<cell> HH:MM:SS`), and the font is HAND-CODED (`src/youtube/label.ts` — a 5×7 glyph table scaled ×3 into a raw PGM strip, `vstack`ed under the cell): `drawtext` needs a libfreetype build absent from this machine's ffmpeg and from the images, and the prose list `selectionPrompt` used to carry alone was measured being applied WRONG (four cells of one sheet mislabelled on both `detailed` runs — 170 called the coding-agent chart when it is experiment-velocity, 130 called a text excerpt when it is the usage-growth chart). The prompt now says the label is authoritative and the row-major list is a cross-check. The read arithmetic, once: ⌈120/12⌉ = **10** sheets for the selection pass, ≤ 2×`maxTotal` = **40** frames for the summary pass, both under `YOUTUBE_FULL_READ_CAP` (60). **The deadline is arithmetic, not machinery** — `executeOneShot` carries no abort signal, so the job states ONE budget (the selection call, the re-grab and a full summary call), bounds every stage out of it (the summary call gets `summarizeTimeoutFor` and never the remainder; the sheets share ONE budget rather than one each; the re-grab has one aggregate deadline), and REFUSES to launch the summary call when the remainder is under its own floor (a visible failure naming the stage) rather than starting one that would overrun. **Two model calls, one job record**: `summarizeVideo` owns the trace root itself (`createCaptureTracer` + the seam's new `parentTracer`, which suppresses the seam's own `finish` — `Tracer.finish` has no idempotence guard), finishes it exactly once on every exit path with both passes' spend summed, and labels the selection span `claude:select` so `/models`' `c.name = 'claude'` join still finds one summary span; `attachRun` now ACCUMULATES the five spend fields while identity stays last-write. Every dense-stage failure below the budget gate falls back to the cadence extractor **on the video already downloaded** and says which stage gave up (`frameScan` on the trace: `dense · cadence · prep_failed · scan_failed · sheets_failed · selection_failed · selection_empty · regrab_failed`). The video now lives in a SECOND temp root (`muninn-youtube-media-<jobId>`) the model is never handed, because the re-grab happens after a model call and the file has to survive one; the job's `finally` removes both roots. The shape of that path is the DURATION: nothing on the route, the job or the transcript endpoint carries one and `downloadVideo`'s own info line arrives after the download it would bound, so it opens with a `yt-dlp -O <YTDLP_INFO_TEMPLATE> --skip-download` probe (~3 s) whose `duration` drives the 3 h cap, the <60 s skip, the frame budget and both timeouts — a probe that says nothing, or a live stream (`parseYtDlpJson` maps a missing duration to **0**, checked BEFORE the length cuts so it is never mistaken for a clip), means frames off + warn + transcript-only. **How much of the video a slides capture may SHOW is a second axis, `visual_detail` on the body** (`selected` — up to 8 visuals inline — or `detailed`, up to 20 in total with the rest under a `## Visual reference` section before `## Transcript`; absent ⇒ `selected`, anything else 400 `bad_visual_detail`, validated beside `frames` and so above the bot, the listing read and `createJob`). The policy and the deterministic pass that HOLDS the answer to it — invented timestamps, another video's frames, non-canonical spellings, repeats and everything past the caps removed before the text is stored, ingested or drafted from — are `src/summaries/visual-detail.ts`, reached through an optional POLICY argument on `framesPromptSection` that leaves Vimeo's prompt byte-identical. **What a quote IS has one definition for the whole seam** (`frameAddressRegExp` + `parseFrameAddress`, `src/summaries/frames.ts`): a markdown image OR link at a `/api/frames/` address, any file name, skipping fenced blocks and inline code (`markdownCodeRegions`) — so what the pass caps is exactly what `keepReferencedFrames` copies (it takes the pass's own list) and what the export rewrites. The appendix is matched as a SECTION in every heading spelling and removed whole where no entry survives, a kept quote's alt is corrected to its own file's second, and the copy is per file, so one frame that cannot be copied costs its own reference and no other. Because the rewrite lands AFTER the summary has streamed, this vertical now sets `completeReplacesText` (state) and `completeCarriesSummary` (route) so a live card and an SSE replay both show the rewritten body. **EVERYTHING stored is built from the validated `video_id`, never `body.url`** — the yt-dlp target, the job, the ingest body, the prompt's `Video URL:`, the source draft and the dedup memory (`summarizeVideo` takes no url at all): the route is CORS-`*` with `MUNINN_AUTH=off` (CORS stays: the entry point IS an extension), a client-supplied URL would let any page spawn yt-dlp at an arbitrary host, and a POST naming video X with a url for video Y made every later capture of Y a `duplicate` of X's document. `video_id` is charset-gated route-wide (400 `bad_video_id`) above the huginn listing read, `frames` is a boolean (400 `bad_frames`), `title` is capped at 300 chars, the body must be `application/json` (415 — a `text/plain` POST is a CORS-simple request that fires the whole pipeline with no preflight) and the connector's `supportsExtraDirs` is pre-flighted (503 `frames_unsupported`) above `createJob`. **The download + ffmpeg half runs under one process-wide queue key**, so N distinct-id POSTs cost one yt-dlp at a time. Frames on ⇒ the transcript is fetched **windowed** (`?timestamps=1`, huginn #129 — `### [HH:MM:SS]` 120 s windows, never sent blank); whether it IS windowed is read off the RESPONSE (`timestamps: true`), never off the frames decision, because a pre-#129 huginn ignores the parameter — and the `## Transcript` section and the windowed rider ride on that answer alone. It is appended to the ingest body's **summary string**, capped at 2 MiB at a window boundary (the note's bytes inside the budget; a first window over it keeps a line-cut head rather than the note alone; a truncation warns with both byte counts), because huginn's YouTube ingest has no `transcript_markdown` field; `completeJob`, the shelf card and the source-page draft get the summary alone — but huginn's `similar` list is ranked on `summary[:2000]`, so it is not in that group. Every frames failure is a warn + transcript-only **except one**: the two-pass BUDGET GATE, where a selection pass that leaves too little of the announced budget for the summary call fails the job with the stage named (nothing can abort an in-flight connector call, so launching it anyway would overrun the announced number silently). The four-state dedup (`inFlight` / `recentIngests` / `recentDeletes` / the listing) is the Vimeo one, and here it is load-bearing rather than cosmetic: **huginn's YouTube ingest keys on the FILE PATH (`<category>/<sanitized title>.md`) and then compares the stored url — same path + same url overwrites, a different url forks `(2)`** — so a re-capture whose auto-category or title differs leaves a SECOND document. **The capture also has a KIND** (`kind` on the route body, absent ⇒ `standard`; non-string, blank or unoffered ⇒ 400 `bad_kind` above the dedup read and above `createJob` — blank is refused rather than folded into "absent", which is a picker that failed to fill): the shared capture presets, narrowed by `resolveCapturePresets`' `requireThinkingControl` argument, which this vertical alone passes and whose whole rationale — plus the accepted consequence that a Copilot summarizer bot is offered `deep` for Vimeo and refused it here — lives once, on that option in `src/summaries/presets.ts`. `youtubeCaptureKinds` (`src/youtube/kinds.ts`) is the ONE resolution the picker, the 400 and the replay harness share. `deep` swaps in `CAPTURE_DEEP_MODEL` through `captureBotConfigFor` BEFORE `runCaptureOneShot` (so the `/agents` card and the trace span say opus from the first frame) and inherits the bot's own budget with slides off, on, skipped or failed — the frames path never touches the model and can no longer reapply the 8 k cap — while `standard` keeps its old rule (the cap unless frames came out). `preset` is REQUIRED on `SummarizeVideoOptions` (the Vimeo precedent — a default would be a positional pick out of the shipped array). The preset id rides the ingest body as `summary_kind` and the trace as `summaryKind`, beside the effective budget; the completion log line names the kind, the connector's OWN reported model and the requested one. **`GET /api/youtube/options`** serves that kind set plus `frames.supported` — `applyCors` like the POST, a CORS-simple GET so it needs no preflight — and the extension popup renders its picker from it rather than from a catalog of its own; an unreadable answer says so and offers Standard only, a remembered kind this instance dropped says so beside the picker, and a disabled Slides tick names the connector rather than sitting there dimmed. The popup's shared rules live in `src/youtube/extension-options-rules.ts` (restore-and-revalidate, the restore note, old payloads, the POST body) and are emitted into `extensions/youtube/capture-rules.js` by `bun run build:extension` (`src/youtube/extension-build.ts`; the emitted bytes are normalized to be cwd-independent, or the byte gate would fail on where it was run). **`bun scripts/replay-youtube.ts`** re-runs a real capture over the summarizer's seams — a local mp4, a saved transcript, a stub huginn, `deps.sourceDraft: false` — to compare kinds without touching the corpus. **Requires `yt-dlp` + `ffmpeg` on PATH for the frames path**; without them a `frames: true` capture warns and degrades to transcript-only, and a frames-off capture never touches either. Details + the traps: `src/youtube/CLAUDE.md`. **Any captured summary can leave muninn as a folder**: `GET /api/summaries/export?source=&docId=` (the ⬇ Export link in the `/summaries` doc panel) answers with a ZIP of a standalone `index.html` plus `frames/<sec>.jpg` for the slides it quotes, rendered by `marked` (the dashboard's own markdown pipeline has no image rule) through `src/summaries/export.ts` + the store-only writer `src/summaries/zip.ts`; rules and guards in the `summaries-export.ts` row of `src/dashboard/CLAUDE.md`.
