...
| `captureMaxItems` | **3** (`DEFAULT_CAPTURE_MAX_ITEMS`, max 25) | **Per-run capture limit K** — the maximum items ONE gate run may admit, applied **after** the per-kind floors. Both a prompt clause (`withCaptureLimit`, appended to the base prompt *before* `withInterestProfile` so the profile block still lands last) and a deterministic post-gate cap (`applyCaptureLimit`). ⚠️ **Degrade direction is the INVERSE of every other capture knob:** absent or invalid ⇒ **ON at the default**, never unlimited — an unbudgeted gate is the defect this exists to fix, so a mistyped value must not silently restore it (contrast `captureAmplifyMin`, where invalid ⇒ off). Sizing: mean `aboveFloor` was **17.9** over 26 measured runs at ~7.7 capture runs/day, so K=3 ⇒ ~23 rows/day against ~138 pre-budget. Wave admissions (step 2b) are deliberately NOT capped — a wave paying out is cross-run bookkeeping, not this run's discretionary pick. See "Capture budget" below. |
...
- **Capture limit (2026-07-29) — the shelf's volume+dedup lever, and WHY it is the right one.** The alert leg and the capture leg run in the SAME `checkX` call, on one fetch, over one window (`x.ts` — capture starts, then `runAlertPath` is awaited, then capture is awaited in `finally`). On 2026-07-29 that pair produced, from identical input: **1** digest line for the 44-tweet "Graph Engineering" hype wave, and **~10** shelf rows. Window, cadence, corpus, ranking and interest-profile are all held constant across that pair, so the gap is attributable to the two things that differ — **model tier** (`config.model` Sonnet 5 for the alert leg vs a hardcoded `DEFAULT_MODEL` Haiku for the gate) and **budget** ("Top Picks (3–5)" vs an absolute floor with no cap). That is a controlled experiment sitting in production, and it **rules out** the global-window-ranking theory that an earlier draft blamed. `captureMaxItems` changes the budget variable alone, on the row capture already runs on — no window, cadence or recall change — so any measured improvement is attributable to it.
  - **Composition order binds: floor FIRST, then budget** (`applyCaptureLimit`'s contract). Budget-then-floor would admit FEWER than K whenever a high-scoring item is floor-blocked (a sub-tier pointer is floor-blocked *unconditionally*), silently under-capturing a strong batch. In the shipped order a weak batch admits fewer than K (correct) and a strong batch admits exactly K. Unit-tested via the kind-floor asymmetry: an `x-post` at 0.74 (floor 0.75) must not consume the slot that an `x-link` at 0.72 (floor 0.7) earns.
  - **Truncation is score-ordered, not parse-ordered.** `parseGateScores` gives no ordering guarantee, so "first K" would be arbitrary; ties break toward the lower index, which is the batch's `combined_score` DESC order.
  - **The prompt clause must forbid padding.** Told only "return at most K", a model reliably returns exactly K — turning a quiet batch into K forced captures. The floors would still hold the bottom, but the shelf would stop reflecting how good the day actually was.
  - **`scores` stays complete through the gate's return.** Admission is expressed as a separate n-set (`CaptureGateResult.admitted`), NOT by stripping non-admitted entries — the step-2b amplifier table records a vote with score/`why` for every pointer the gate saw, including floor- and budget-rejected ones, and stripping would silently hollow it out.
  - **Log fields:** `aboveFloor` stays **pre-budget** so the historical series spans the ship date; new `budget` and `admitted` (= `min(aboveFloor, K)`) carry the post-budget number the shelf's daily volume is mined from.
  - ⚠️ **`aboveFloor` and `scored` are NOT comparable across this ship date.** The limit is a PROMPT clause as well as a deterministic cap, so post-limit the model returns at most K items *at all* — which caps `scored`, and therefore caps `aboveFloor`. Pre-limit runs measured mean `aboveFloor` **17.9**; post-limit it is structurally ≤ K. **Only `n` (items FED to the gate) spans both eras.** The pre-limit distribution is recorded here as the baseline — do not try to re-derive it from post-limit lines, and do not read `aboveFloor − admitted` as "items displaced by the cap": that difference can now only reflect floor rejections among the ≤K returned items.
  - **Budget-rejected items are dropped PERMANENTLY, not deferred.** `trackingIds` rides the full fetched batch and is persisted into `lastNotifiedIds` on every non-error exit, and (on `dedupByTweetId: true`, which the live capture row runs) the next fetch filters on it. So an item that clears its floor but loses the limit race is never reconsidered — this is a *limit*, not a throttle. Same semantics as a floor rejection, but worth knowing before raising K to "catch up".
  - **Step-2b interactions (dormant — `captureAmplifyMin` unset — but on its enablement checklist):** (a) a pointer that cleared its floor and lost only the LIMIT race must never queue a wave check, or it would get an uncapped same-run admission through the path that deliberately isn't capped — enforced by keying the queue on `passedFloor`, and it is why `decideAdmissions` returns both sets; (b) the prompt clause means most pointers now get **no score at all**, so their `x_link_amplifiers` votes are NULL-score. `getAmplifierGroup` picks `best` only from non-NULL rows, so a legitimate 3-author wave can accumulate and still never admit. Re-check this before enabling 2b.
  - **Watch:** whether the largest single wave drops to ≤2 rows; whether K=3 lands in the 15–25 rows/day band; and whether Rune judges a good item missing from the shelf (the only available displacement signal — see the comparability note above). If so, raise K before weakening the floors.
...
- **Capture-gate calibration (2026-07-26, X hype-dedup step 1).** The `/summaries` candidate shelf was wall-to-wall ALL-CAPS engagement-farm pointer tweets at 0.9–0.95: the displayed score is verbatim the gate output, and the prompt's only anchors were ~1.0 / ~0.7 / ~0.6 — nothing separated 0.9 from 0.95 (measured: corr(gate score, `author_score`) = **−0.001** over 659 candidates; histogram 0.95×41 / 0.90×64 / 0.85×140 / 0.80×122). The frame verdict was that the pointer *content* is wanted — the problem is redundancy + credibility-blind ranking, **not** topical misfire — so this is a calibration change, not a harsher relevance cut. Three prompt edits, asserted in `x.test.ts`:
  ...
  - **Real 0.8/0.9 anchors + in-list dedup.** ≥0.9 is RESERVED for a **primary source ONLY** — the artifact itself (the author's/destination's own work, paper, repo, release, announcement), never coverage of one — so the UI's ≥0.9 "high" band is **biased toward** artifacts. Prompt compliance is best-effort — an identical item scored 0.85 on one run and 0.9 on a straight re-run — and the plan's stated fallback was a **deterministic post-gate clamp** if the first week of `ok` lines showed poor compliance. Step 1 shipped none; the measurement came in and the clamp **shipped 2026-08-19**, so **the 0.8 repackaging cap is now enforced in code** (see "Repackaging clamp" below). The **0.85 non-primary-source ceiling remains prompt-only** — nothing clamps it. Loud framing/ALL-CAPS/engagement volume are explicitly named as non-evidence (ceiling 0.85 without that standing); 0.8 = strong secondhand coverage (and the repackaging ceiling).
...
- **Repackaging clamp (2026-08-19) — the deterministic half of the step-1 repackaging cap above.** The prompt asks for a ≤0.8 ceiling on secondhand long-form and does not get it. Measured on prod 2026-08-19 over `source='x' AND created_at >= '2026-07-29 12:00'` — **375** rows, all `x-post`: **95** carry the shape, **84** of them stored strictly above 0.8 (0.90×38, 0.85×28, 0.88×6, 0.92×4, 0.95×2, and one each at 0.81/0.82/0.83/0.84/0.87/0.89) and 9 stored at exactly 0.8. Examples: `@vicky_grok: EXO JUST SHOWED HOW SELF-IMPROVING AI AGENTS ACTUALLY WORK.` 0.92, `@Dipanshu_AI: Anthropic just released a 4-hour course…` 0.88. Corpus-wide the shape is **374 of 1453** `x-post` rows (25.7%). The clamp (`clampScores` in `gate-scores.ts`, shape + cap in `repackaging-shape.ts`) is the fallback the step-1 prompt docblock always named. **This block owns those numbers** — the code comments point here rather than restating them.
  - ⚠️ **`score` is `float4`, so the acceptance query must round.** `score > 0.8` in Postgres compares the stored `float4` (0.800000012…) against numeric `0.8` and is **TRUE for a row stored at 0.8** — that is where an earlier "93 of 95 above 0.8" came from. Judge the clamp with `round(score::numeric, 2) > 0.8`, which is how the 84/9 split above was measured.
  - **Shape**, on the candidate's **handle-stripped title** — `candidateTitle(doc)` minus its `@handle: ` prefix, i.e. whitespace-collapsed and truncated at 140 chars, the exact slice the census scored: an ALL-CAPS run of ≥8 letters, a leading 🚨, or `just (dropped|released|launched|published)`. The census SQL, corrected to the same handle-stripped form the code applies, is `regexp_replace(title,'^@\S+:\s*','') ~ '[A-Z]{8,}' or regexp_replace(title,'^@\S+:\s*','') like '🚨%' or title ~* 'just (dropped|released|launched|published)'`. It is NOT the census predicate *verbatim*: the census ran `title like '🚨%'` against full titles, which all start `@handle: `, so that clause was **dead** (0 matches) — in the code it is live — 8 stripped in-window titles open with 🚨, and for **3** of them (@Nova0xz, @anyelamarillo, @cyrilXBT) that clause is the ONLY one matching, all plainly hype. Judging `gateExcerpt` instead of the title would over-match (1200 chars eventually contain an acronym run), and an **empty first line is never clamped** — `candidateTitle` then falls back to the 500-char compact digest, which carries a second `@handle:` prefix and the tweet URL, so an ALL-CAPS handle would clamp itself on text no census ever scored.
  - **Known cost, stated honestly.** **0 of the 95** in-window shape rows were ever summarized. But corpus-wide, **2 of the 9 X rows ever accepted DO carry the shape** — `@Sprytixl: ANTHROPIC JUST LEAKED…` (0.95, 2026-07-04) and `@0xMovez: Andrew Ng just dropped 12-page PDF…` (0.95, 2026-07-25). Both predate the window; both would have been clamped to 0.8 and ranked lower (not necessarily dropped — 0.8 clears the 0.75 non-top floor). That is the false-positive rate the widening ban is protecting, not a clean zero.
  - **Long-form `x-post` only; `x-link` pointers are NEVER clamped.** Same carve-out as the prompt's, for the same reason: destination rows inherit the representative pointer's score, so capping pointers would pin every promoted artifact below the inbox's 0.85 cut while uncapped hype sat above it.
  - **Applied right after `parseGateScores`, before `indexScoresByN`/`decideAdmissions`** — so it shapes the persisted score AND the K-slot race in `applyCaptureLimit`. A clamped 0.8 note now loses to a 0.9 primary source; that displacement is the mechanism behind "at least one row names the artifact rather than a tweet about it". Never raises; `why` is untouched. `clampScores` itself takes the cap as a REQUIRED argument (`REPACKAGING_SCORE_CAP` lives with the predicate, not with the generic helper) so the anthropic caller cannot inherit an X ceiling by accident.
  - ⚠️ **An x-post floor above 0.8 turns the clamp from a re-ranker into a class ban.** With `candidateMinScoreByKind["x-post"]` (or `candidateMinScore`) set above 0.8, every shaped item lands exactly at the cap and fails the floor, so the whole class stops being captured rather than being ranked lower. The defaults (0.6 base, 0.75 non-top raise) are safe.
  - **New writes only.** `upsertCandidate` keeps `GREATEST(stored, incoming)`, so the 84 existing above-cap rows keep their pre-clamp highs — clearing them is a separate decision.
  - **Live home of the organic metric: the `/summaries` Calibration tab.** Its "Last N days" block (`GET /api/anthropic/candidates/stats?days=`, `candidateRecentStats`) renders `repackagingShapedAbove08` per window — **`kind='x-post'` rows only** whose handle-stripped title is shape-matched by the SHIPPED `isRepackagingShaped` and whose `round(score,2)` is still above the cap. **Target 0**; it runs the predicate in TS over the window's rows rather than re-stating the three clauses as SQL, so the dashboard cannot drift from the code — and for the same reason it mirrors `clampScores`' other two exclusions clause for clause: `x-link` pointers (never clamped, by design) and the empty-first-line fallback (a title still starting with `@\S+:` after one strip). Counting either would pin the target-0 metric permanently above zero for reasons that are not misses; prod carries 3 x-link rows today, 0 flagged, latent. **The count is floored at `REPACKAGING_CLAMP_SHIPPED_AT`** (`2026-08-19T12:04:10Z`, the merge of #454, declared beside `REPACKAGING_SCORE_CAP` in `repackaging-shape.ts`): the "new writes only" rule above cuts both ways — `GREATEST(stored, incoming)` keeps every one of the 84 pre-clamp highs forever, so a count over rows captured before the clamp measures the ratchet rather than the clamp, and a 90-day window would report a permanent non-zero that no fix can move. The payload states the effective floor (`recent.repackaging = {cap, since, floored}`) and the tab says so under the table when `floored`. The hand-run census SQL above stays the auditing form; the tab is what to look at day to day.
  - **Log-side companion:** the `x-capture-gate ok` line carries `clamped=<n>` — DISTINCT `n` lowered (an off-contract duplicate entry must not print `clamped > scored`), `ok` outcomes only, same rule as `scored`.
