...
- **Repackaging clamp (2026-08-19) — the deterministic half of step 1's repackaging cap.** The gate prompt asks for a ≤0.8 ceiling on secondhand long-form; over the 21 days after #399 (380 rows) **93 of 95** repackaging-shaped `x-post` rows sat ABOVE it (`@vicky_grok: EXO JUST SHOWED HOW SELF-IMPROVING AI AGENTS ACTUALLY WORK.` 0.92, `@Dipanshu_AI: Anthropic just released a 4-hour course…` 0.88). The clamp (`clampScores` in `gate-scores.ts`, shape in `repackaging-shape.ts`) is the fallback the prompt docblock always named.
  - **Shape = the census predicate verbatim**, on the candidate's FIRST LINE without the `@handle: ` prefix: an ALL-CAPS run of ≥8 letters, a leading 🚨, or `just (dropped|released|launched|published)`. That predicate's false-positive record on real data is clean — **0 of 96** shape rows were ever summarized, and neither of the 2 rows Rune accepted carries the shape — which is exactly why it must not be widened. Judging `gateExcerpt` instead of the first line would over-match (1200 chars eventually contain an acronym run).
  ...
  - **Applied right after `parseGateScores`, before `indexScoresByN`/`decideAdmissions`** — so it shapes the persisted score AND the K-slot race in `applyCaptureLimit`. A clamped 0.8 note now loses to a 0.9 primary source; that displacement is the mechanism behind "at least one row names the artifact rather than a tweet about it". Never raises; `why` is untouched.
  - **New writes only.** `upsertCandidate` keeps `GREATEST(stored, incoming)`, so the 93 existing rows keep their pre-clamp highs — clearing them is a separate decision.
  - **Log-side companion:** the `x-capture-gate ok` line carries `clamped=<n>` (entries LOWERED), `ok` outcomes only, same rule as `scored`.
...
- **Capture-gate calibration (2026-07-26, X hype-dedup step 1).** The `/summaries` candidate shelf was wall-to-wall ALL-CAPS engagement-farm pointer tweets at 0.9–0.95: the displayed score is verbatim the gate output, and the prompt's only anchors were ~1.0 / ~0.7 / ~0.6 — nothing separated 0.9 from 0.95 (measured: corr(gate score, `author_score`) = **−0.001** over 659 candidates; histogram 0.95×41 / 0.90×64 / 0.85×140 / 0.80×122). The frame verdict was that the pointer *content* is wanted — the problem is redundancy + credibility-blind ranking, **not** topical misfire — so this is a calibration change, not a harsher relevance cut. Three prompt edits, asserted in `x.test.ts`:
  ...
  - **Real 0.8/0.9 anchors + in-list dedup.** ≥0.9 is RESERVED for a **primary source ONLY** — the artifact itself (the author's/destination's own work, paper, repo, release, announcement), never coverage of one — so the UI's ≥0.9 "high" band is **biased toward** artifacts. Best-effort, not enforced: there is **no code-side ceiling or clamp** on the returned score, and an identical item scored 0.85 on one run and 0.9 on a straight re-run. The plan's stated fallback if the first week of `ok` lines shows poor cap compliance was a **deterministic post-gate clamp**; it was not shipped in step 1 and **shipped on 2026-08-19** once the measurement came in — see "Repackaging clamp" above. The 0.85 ceiling remains prompt-only. Loud framing/ALL-CAPS/engagement volume are explicitly named as non-evidence (ceiling 0.85 without that standing); 0.8 = strong secondhand coverage (and the repackaging ceiling).
