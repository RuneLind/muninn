...
| `email` returning `[]` | **Fixed (2026-08-13)** — the liveness predicate this row asked for now lives in `checkEmail`, reading `spawnHaiku`'s returned `toolCalls` rather than the tracer: zero `mcp__gmail__*` calls ⇒ **throw**, because there is no path to a truthful answer that skips `search_emails`. That converts the silent case into the runner's ordinary loud failure. Note what it does NOT do: `recordOutcome` still can't be applied naively here ("no important email today" remains the modal *correct* output, so 3 quiet hours would escalate) — the predicate distinguishes **reached Gmail and found nothing** from **never reached Gmail**, which is the distinction health was missing. `toolCalls` undefined (legacy-JSON parser fallback) means "can't tell" and never fires. Measured on the live row the day it shipped: 3 of 7 organic ticks had been returning `[]` without calling Gmail. |
