...
| `candidateMinScore` | 0.6 | Inbox capture floor for **top-5%-author** tweets — long-form tweets scored ≥ this by the capture gate are queued. Independent of the alert `minScore`. |
| `candidateMinScoreNonTop` | 0.75 | Stricter capture floor for **non-top-5%-author** tweets (unknown authors, and — deliberately — EVERY author when the author-scores file is unavailable). Effective floor is `max(candidateMinScore, candidateMinScoreNonTop)`, raise-only. Author tier is resolved once per run from huginn's `x-feed-author-scores.json` percentile cuts (`getAuthorTierThresholds`); the tier (top 1% / top 5%, never the raw score) is also injected into the capture-gate prompt as an author-rank prior. Degrade direction is safe — scores-file outage ⇒ fewer captures, never a silent widening. |
