...
export type JobEvent =
  ...
  // summary so a live browser can swap out the chatter (see completeJob).
...
const jobs = new Map<string, TikTokJob>();
...
export function getJob(jobId: string): TikTokJob | undefined {
  return jobs.get(jobId);
}

export function getRecentJobs(limit = 20): TikTokJob[] {
  return [...jobs.values()]
...
export function completeJob(jobId: string, summary: string, category: string): void {
  ...
  job.summary = summary;
  job.category = category;
  // Replace the accumulated stream (which for TikTok includes inter-turn "let me
  // read frame N" chatter) with the clean parsed summary, so an SSE *replay* of
  // job.text shows only the summary. The complete event below carries the same
  // summary to clean a *live* browser that already accumulated the chatter.
  job.text = summary;
  publish(jobId, { type: "complete", summary });
  log.info("TikTok job {jobId} completed, category: {category}", { jobId, category });
}
...
export function failJob(jobId: string, error: string): void {
  ...
  log.error("TikTok job {jobId} failed: {error}", { jobId, error });
}
