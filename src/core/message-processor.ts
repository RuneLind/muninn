...
export async function processMessage(params: ProcessMessageParams): Promise<ProcessMessageResult | undefined> {
  ...
  // THIS PIPELINE's one log line carrying message CONTENT — an 80-char preview
  // plus the username. (The Slack handlers, `voice-handler.ts` and
  // `response-handler.ts` log previews of their own; they are not reached from
  // here and are a follow-up.) On a personal instance that is the operator
  // reading their own chat; on the nais profile the pod's stdout is a shared log
  // aggregator and the previews are colleagues' messages, so the line drops to
  // `debug` (off by default) there.
  //
  ...
  // at all. The `activity_log` push above keeps the FULL text: that row is
  // admin-zone by design and is how an operator reconstructs a turn.
  const preview = text.slice(0, 80) + (text.length > 80 ? "..." : "");
  const previewLevel = config.profile === "nais" ? "debug" : "info";
  log[previewLevel]("Message from {username}: \"{preview}\"", { ...props, preview });
