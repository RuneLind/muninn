...
2. The EMPHASIS family (`*`, `_`, backtick) is checked by run PARITY instead of count equality; the BRACKET family (`[`, `]`, `(`, `)`) keeps count equality in both modes, because wrapping half a `[label](url)` puts the tag inside link markup. Both families are DERIVED from `RESCUE_DELIMS`, so a delimiter added there lands in one or the other.
3. `trimCutReason` re-checks the span `factSpanForm` actually settled on, but ONLY when it TRIMMED one: `longestLineRange` can cut a balanced multi-line `**Bold text\nmore**` back to `**Bold text`, which was written to the page as two literal asterisks. It compares per-delimiter run parity **against the pre-trim span, never against zero** — an absolute "even runs" rule was tried and refused every passage carrying `user_id`, `2 * 3` or a glob, none of which is emphasis. An UNTRIMMED span is the range the rescue gate already validated and is left alone; a wikilink EXPANSION is likewise not a trim and carries its own rule (a `[[Page]]` inside backticks is a live link the reader resolves — mark it whole, odd backtick count and all). Marking strictly inside a code span is untouched and remains the follow-up `integrate-wikilink.test.ts` names.
