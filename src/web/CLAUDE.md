...
3. `finalSpanCutReason` re-checks the span `factSpanForm` actually settled on. That is not the range the rescue gate saw: `longestLineRange` can trim a balanced multi-line `**Bold text\nmore**` back to `**Bold text`, which was written to the page as two literal asterisks. It runs at every tier, so it also refuses a mark landing inside a construct an EXACT match found. **A span `expandOverWikilinks` widened to a whole `[[…]]` is exempt from all of it** — see the backticked-wikilink rule below: that link is live, odd backtick count and all, and refusing it would trade durable damage for a cosmetic one.
