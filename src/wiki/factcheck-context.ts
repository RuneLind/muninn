...
function factcheckVerifySystemPrompt(): string {
  return [
    ...
    "- A `Sources:` line listing ONLY the source(s) you actually opened with WebFetch for this claim, each as a markdown link `[hostname](url)` (e.g. `Sources: [nature.com](https://www.nature.com/articles/x), [who.int](https://www.who.int/news/y)`), comma-separated. Omit the line entirely when you opened none (and then the verdict cannot be ✅).",
  ].join("\n");
}
