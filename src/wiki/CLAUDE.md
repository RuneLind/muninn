...
showing `created X · updated Y` in full. Past the relative window the day comes
from the signal's own label (`pageDateLabel` / `pageAddedLabel`) rather than from
the stamp, so an authored frontmatter date is echoed verbatim on both surfaces.
