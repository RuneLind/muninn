...
describe("parseFrameQuotePath — what is an address this capture can serve", () => {
  test("the current spelling of this source", () => {
    expect(parseFrameQuotePath(`/api/frames/youtube/${YT_ID}/137.jpg`, YOUTUBE_FRAME_SOURCE)).toEqual({
      id: YT_ID,
      sec: 137,
    });
  });
  ...
  test("the legacy prefix, where the source declares one", () => {
    expect(parseFrameQuotePath("/api/vimeo/frames/1223642971/90.jpg", VIMEO_FRAME_SOURCE)).toEqual({
      id: "1223642971",
      sec: 90,
    });
    // YouTube has no legacy spelling, so the same path addresses nothing.
    expect(parseFrameQuotePath("/api/vimeo/frames/1223642971/90.jpg", YOUTUBE_FRAME_SOURCE)).toBeNull();
  });
  ...
  test("null for every path the route would not serve", () => {
    for (const path of [
      ...
      expect(parseFrameQuotePath(path, YOUTUBE_FRAME_SOURCE)).toBeNull();
    }
  });
});
