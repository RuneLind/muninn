...
## The `Vimeo` entry from the root `CLAUDE.md` module table

Conference-talk capture — a Vimeo URL to a summarized, indexed, citable talk, into the `vimeo-summaries` collection. `url`/`oembed`/`vtt`/`captions` are PR 1's transcript core (headless Chromium harvests the signed VTT the real player fetches — read `src/vimeo/CLAUDE.md` before touching that, and note `bunx playwright install chromium` is an operator step on **both machines**); `state.ts` + `summarizer.ts` + `dashboard/routes/vimeo-routes.ts` are the vertical. **The media seam** (v2 PR 3, `src/vimeo/media.ts` + `download.ts`): the signed JSON manifest the harvest records as `manifestUrl` (cookie-less, host-pinned to the `VIMEO_MEDIA_HOSTS` allowlist — `vod-adaptive-ak.vimeocdn.com` and `skyfire.vimeocdn.com`, the player picks one per video — under the `downloadVtt` rules, which now live ONCE in `downloadPinned`) → `chooseRepresentation` (pure: smallest rendition at least the target height; Opus for audio) → `downloadRendition` writes init + any segment set, sparse or contiguous, as ONE fMP4 with ABSOLUTE timestamps that ffmpeg reads directly. Four bounds (manifest 8 MB, segment 16 MB, declared total 256 MB, whole-operation budget), partial files unlinked. `scripts/smoke-vimeo-media.ts` is the live proof. **Whisper fallback** (v2 PR 5, `src/vimeo/whisper.ts`): a video with NO usable caption track and a manifest is transcribed from its whole Opus rendition (`whisper-cli -l auto -ovtt`, the VTT then windowed exactly like a caption track), `caption_kind: "whisper"` + whisper's detected language on the document so `talk` still resolves; the harvest holds the page for the manifest only when the player lists no track (`awaitManifestNoCaptionsMs`); three stable job codes (`whisper_unavailable` — pre-flighted BEFORE the download, `transcription_failed`, `no_speech`), and `VIMEO_WHISPER_MODEL_PATH` must name a MULTILINGUAL model (the shared default is English-only; operator step: `brew install whisper-cpp` + `ggml-small.bin`). **Slides** (v2 PR 4, `src/vimeo/frames.ts`): a `frames` boolean on the route body (off by default; the picker's Slides checkbox, remembered under the same key) — 503 `frames_unsupported` BEFORE oEmbed where the summarizer's connector lacks `supportsExtraDirs`, and the checkbox renders disabled there — pulls one 720p frame per `frameBudgetFor` cadence tick (slice midpoints) through the seam, hands them to the model via `extraDirs` with the TikTok frame list plus a rider to quote a slide as an image IN PLACE (`![Slide at HH:MM:SS](/api/frames/vimeo/<id>/<sec>.jpg)`, ≤8), keeps ONLY the quoted frames under `~/.muninn/frames/vimeo/<videoId>/` and serves them read-only from `GET /api/frames/:source/:id/:file` (every segment charset-gated — the SOURCE is default-deny and the id is gated by THAT source's charset — 404 otherwise). **The source-neutral half of all of that lives in `src/summaries/frames.ts`** (the id gate as an invariant of the seam, the served root, the URL shape, the prompt section, the kept-frame copy/removal, the ffmpeg argv, and `extractCadenceFramesFromFile` for a vertical holding the whole video on disk); `src/vimeo/frames.ts` keeps only `VIMEO_FRAME_HEIGHT` and the manifest-shaped `extractCadenceFrames`, and the dependency is one-way. The route is `registerFramesRoutes` (`src/dashboard/routes/frames-routes.ts`), registered INSIDE the `summaries` group — vimeo, youtube and summaries are dropped together on nais — and it also serves the pre-seam alias `GET /api/vimeo/frames/:videoId/:file` over the same root, because every Vimeo document ingested before the seam quotes that path and huginn stores the summary markdown verbatim (`referencedFrameSeconds` accepts both spellings for the same reason). `~/.muninn/vimeo-frames` is moved to `~/.muninn/frames/vimeo` by a one-time rename in `src/index.ts` — never at module load or route registration, where a test would perform it under the developer's real `$HOME` — skipped on nais and refused with a warn when both roots exist. The harvest waits up to 10 s for the player's manifest request when frames are on (captions arrive first). Every frames failure (no manifest, extraction error) is a warn + transcript-only, never an error job. Cadence only — scene detection waits on the plan's skip trigger. **The UI entry is the always-visible URL field on `/summaries`** (`views/components/sum-submit-form.ts` — `captureUrlFormHtml` + `submitCaptureUrl`), the one capture vertical with no Chrome extension: it POSTs `{url, kind, lang, frames}` to the route and reuses the page's ordinary job card + SSE, and a Vimeo link pasted into the ARTICLE textarea is forwarded to the same submit (every other bare link keeps the byte-identical extension alert). **The kind + language picker** (v2 PR 1) sits beside the field: kinds are the SUMMARIZER bot's resolved preset set (`src/summaries/presets.ts` — shipped `standard` · `deep` (opus + the bot's full thinking budget) · `talk-notes` (a `## Timeline` of `### [HH:MM:SS]` blocks), plus per-bot `prompts/captureSummary.<id>.md` — and narrowed by that bot's CONNECTOR: `deep` is not offered, and is 400'd, where the connector cannot run the opus model), languages are `talk | nb | en` (`src/summaries/language.ts`; `talk` is the default and resolves in the SUMMARIZER from the TRANSCRIPT's own function words first (`detectTextLang`) and the chosen caption track's base tag second, `no`/`nb`/`nn` ⇒ bokmål — the tag alone mis-labelled a Norwegian talk `en-x-autogen`, measured), both remembered per browser under `muninn.summaries.capture.v1`, both validated in the route BEFORE oEmbed (unknown ⇒ 400 `bad_kind`/`bad_lang`, a sentence on the card), and both written on the document as `summary_kind` + `summary_lang` (the RESOLVED language). The bokmål/English rider is ONE function, owned by `src/summaries/language.ts` and re-exported by `src/share/prompt.ts`; it is the LAST line of the system prompt. **What oEmbed knew is on the document too** (v2 PR 2, huginn #127): `author`, `upload_date` (its own key — `date` stays the capture day), `speaker` (`src/vimeo/metadata.ts` — the last ` - ` segment of a title uploaded by an account in `VIMEO_CONFERENCE_ACCOUNTS`, JavaZone today; nothing for anyone else) and `thumbnail_url`, each sent only when non-empty; the shelf card renders the poster frame off the listing's `include_thumbnails=1` (https only, lazy), and the article view turns every `[HH:MM:SS]`/`[MM:SS]` of a Vimeo capture into a `vimeo.com/<id>#t=<sec>s` link (`linkVimeoTimestamps`, fenced code and existing links left alone). Details: `src/vimeo/CLAUDE.md`. Every route answer that is not a fresh job renders as a SENTENCE on the card from ONE map in `sum-job-card.ts` (`vimeoSentence` — `bad_url`/`not_public`/`duration_unknown`/`too_long`/`oembed_failed`/`no_captions`/`duplicate`/`in_flight`), which the job-error path reads too, so the `no_captions` CODE the job stores never reaches a reader as the word itself. **oEmbed runs in the ROUTE, before a job exists** — not public ⇒ 422 `not_public`, **no duration ⇒ 422 `duration_unknown`** (`toDurationSec` degrades a missing/non-numeric/negative value to 0, which passes the cap unconditionally, and the duration is what the cap, the frame budget and the summarize timeout are all sized from), over the 3 h cap ⇒ 413, already captured ⇒ a `duplicate` body with no `job_id`, **already capturing ⇒ an `in_flight` body carrying the running job's id** — so there is no `fetching_metadata` status and no early return can leave an unsettled row at the top of `/summaries`; the metadata is handed to `summarizeVimeo`, which never asks again. **A video id is in one of FOUR states and dedup needs a guard for each** — enumerated, because a state with no owner is a video captured twice: (1) absent everywhere ⇒ capture; (2) claimed in-flight in this process ⇒ the `inFlight` map answers `in_flight` with the running job's id, since nothing is stored until a capture finishes and huginn suffixes `(2)` rather than overwriting; (3) **ingested here but not yet LISTED** ⇒ the `recentIngests` map; (4) listed by huginn ⇒ `findExistingByVideoId`, which resolves each listed row's url to its VIDEO ID (`resolveVimeoRef(d.url)?.id`, the youtube sibling's rule — the live collection holds rows this route never wrote, and `vimeo.com/<id>/<hash>` is the same video). **State 3 is huginn's REINDEX WINDOW and it was owned by nothing:** the `/documents` listing is derived from `index_document_mapping.json`, which moves only when the reindex enqueued after an ingest has run — seconds to minutes later — while the in-flight claim is released the instant the capture settles, so a re-POST of a just-completed capture started a SECOND job and spent a second model call, invisibly (huginn's writer overwrote the same category/title/url, so only one document remained). `recentIngests` (`videoId → {documentId, existingUrl, at}`) is written from the `onIngested` hook `summarizeVimeo` calls on a successful ingest — the only moment the process knows a document exists — read BEFORE the listing, and bounded on BOTH axes because its only invalidation is the delete signal: `VIMEO_RECENT_INGEST_TTL_MS` (30 min) and `VIMEO_RECENT_INGEST_MAX` (200, oldest-inserted evicted); past either the listing is authoritative again and the worst case is a re-capture. **A `/summaries` Delete forgets the entry at once** — `backlog-doc-delete` calls `notifySummaryDocumentDeleted` (`src/summaries/document-deleted.ts`, a module-level listener set, since the two routes share no handle) AFTER huginn confirmed the move, and the Vimeo registration drops the entries naming that document id AND remembers the id in a mirror map, `recentDeletes` (same TTL and cap), because huginn's DELETE is a SOFT delete whose listing keeps naming the document until its reindex lands — so the listing half treats such a row as absent until a capture ingests under that id again. Before that, a capture deleted and re-pasted inside the TTL was answered `duplicate` with a link to nothing, from either half. A document removed on the huginn side directly is not announced and stays in the map for the TTL. The same listener removes the deleted document's kept frames under `~/.muninn/frames/vimeo/<videoId>/` (video id from the ingest map, else from the listing row huginn still serves) through the shared `removeKeptFramesForDocument`, called from INSIDE that one listener rather than as a second listener of its own: the dedup half DELETES the ingest entry, so a second listener running after it would send every delete down the listing fallback. Both halves answer one shared `duplicateBody`, so the reader cannot tell which did. **Every harvest goes through ONE module-level `createQueue()` key**, so two pastes are two jobs and one Chromium. The chosen track's `detectCaptionKind` drives a proper-noun rider on the prompt (auto-captions garble names — "JavaBeen" for JavaBin, measured) and rides the document as `caption_lang`/`caption_kind`; the windowed transcript is ingested under a `## Transcript` heading, each window opening a `### [HH:MM:SS]` HEADING (huginn's `MarkdownHeadingSplitter` carries the nearest heading into every chunk; with bare bracketed lines 48 of 75 measured chunks had no timestamp at all) so a hit in a 50-minute talk cites to the minute, while staying out of huginn's ingest response and similarity query; the `/summaries` article view cuts the stored markdown at that heading (`splitTranscript`, fence-aware, level 2 only) and folds the tail into a closed `<details>`, and the quoted slides survive into the stored copy only because huginn's file converter keeps ordinary images in the document `text` (huginn #128 — the chunk text still drops them). **No CORS** (`corsPreflight: false`, no `applyCors`): there is no extension, and a cross-origin summarize entry spends the operator's model budget. Dropped on `MUNINN_PROFILE=nais` with the other capture verticals — the binary there is Chromium rather than yt-dlp. `VIMEO_HARVEST_STUB` replaces the whole browser half with a local `.vtt` for acceptance runs, behind three required gates (default profile, absolute path, file exists) that each degrade to a warn plus a real harvest; **every** stubbed capture warns with the fixture and the job id, and its document carries `caption_kind: "stub"` so it can never pass for a live capture.
