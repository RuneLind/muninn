...
| `MUNINN_AGENT_CWD` | No | `~/.muninn/agent-cwd` | Root for the cwds muninn's own `claude -p` spawns use (`src/ai/agent-cwd.ts`, one subdir per bot). A cwd inside the repo makes the CLI load muninn's CLAUDE.md + `.claude/` surface into the call (~18k prompt tokens, measured) and file the transcript into the developer's own `~/.claude/projects/-…-muninn/` folder. Applies **only to `spawnHaiku` calls with no explicit cwd** — the x/anthropic watchers, the gardener cluster/map calls, the interest-profile distillation. Callers that pass `bots/<name>/` on purpose (email watcher → Gmail MCP, memory/goal/schedule extractors + prose reminders → persona) are unchanged and, since bot dirs sit inside the repo, still pay the CLAUDE.md load — cutting that needs `--mcp-config` + `--system-prompt` (what `src/ai/executor.ts` does for chat), not a cwd change. An override resolving inside the checkout is refused with a warning. |
