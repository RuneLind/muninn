...
| Plans | `src/plans/` | The `/plans` board over mimir's `plans/` directory, priced from the claude-usage ledger. `source.ts` reads the frontmatter + `queue.yaml` off disk, `ledger.ts` proxies claude-usage's `GET /api/plans`, `estimate.ts`/`join.ts` price and join them, and `board.ts` assembles the payload `GET /plans` embeds and `GET /api/plans/board` serves (route: `dashboard/routes/plans-routes.ts`, view: `dashboard/views/plans-page.ts`, client rules in the DOM-free `board-client-pure.ts`). **Two write endpoints** edit the mimir wiki: `POST /api/plans/priority` upserts one plan's frontmatter `priority:` through `writeWikiPage` in its new **no-log mode** (`logKind: null` — a triage sitting is a burst of clicks, so no `log.md` line and no huginn reindex; the read cache is still refreshed), and `POST /api/plans/order` writes `plans/queue.yaml` through `src/plans/write.ts` (`writeWikiPage`'s path confinement admits `.md`/`.mdx` only), joining the same per-wiki write queue with the same sha256 CAS. **Neither commits** — mimir is in the repo-sync loop in `wiki` mode and the loop is the committer. **Both return the NEW hash, taken from the string that was written**, never from a re-read after the section released; without it the second edit on one card 409s. The priority transform is FENCE-SCOPED and fails closed (`src/plans/frontmatter.ts`): a whole-file `^priority:` upsert edits plan bodies (`mimir-plan-status-lifecycle.mdx` carries `plan_status:` inside a fenced ```yaml example). Clearing has exactly one wire form, `"clear"`; an omitted or null `priority` is a 400. `queue.yaml`'s base hash has three states — `""` = absent (accepted only against an absent file, the bootstrap), a real hash, and the reader's `null` = present-but-unreadable, which the writer REFUSES. Priorities and hand orders in the browser are still a `localStorage` draft until PR 5 wires them to these routes; disk wins on every load. **A column's hand order is a ranked PREFIX, not a permutation** — ▲ on an unranked card appends it, ▼ on the last ranked card un-ranks it, and a column `queue.yaml` ranks is read-only (its draft is dropped on load), so ranking six cards in a 49-card column leaves 43 degrading on priority-then-age. View state (scope/sort/query/repo/priority) lives in the URL. **Every meter COUNT is over the cards the current scope renders** (`cardsInScope`) — a follow-ups tile counting a column no scope shows is a number the reader cannot check, so it renders `—` rather than `0` there — while **Spent to date stays corpus-wide** (all scopes, filters applied) and says so in its note, since under the default scope no terminal column is on screen at all. An unreachable ledger hides the MONEY only — columns, priorities and order still render, repo families fall back from PRs to slugs (the banner says so), and neither route can 5xx: the assembly AND the render are caught. |
