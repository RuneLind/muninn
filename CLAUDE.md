...
| Video media | `src/video/media.ts` | Shared source-agnostic video engine (moved from `src/tiktok/`) — yt-dlp download (duration cap is **required**, no default: each vertical states its own, since the shared 600s one silently capped TikTok at 10 min), whisper transcription, ffmpeg keyframe extraction, plus the per-host URL helpers (`extractTikTokVideoId` — now host-gated so X's `/video/1` media-slot suffix can't misfire — `extractXStatusId`, `canonicalXStatusUrl`). Used by the TikTok and X-video verticals. The frame budget GROWS past 10 min (~40s spacing up to `FRAME_BUDGET_MAX` = 60) rather than staying flat at 30 — a flat 30 sampled a 60-min tutorial once per 124s, measured — and that ceiling is one shared constant, because the same number as a second literal inside `extractKeyframes` is what made the first raise inert. `summarizeTimeoutFor` scales the summarize call with the frame count (600s floor + 20s per frame past 30). |
