...
| `wikiCollections` | string[] | — | Huginn search collections backing this wiki's `/wiki` **Ask** tab (research-style cited Q&A scoped to the wiki via `GET /api/wiki/ask`, reusing the `/research` pipeline). Validated at discovery (non-array or non-string element ⇒ warn + drop). Plumbed onto the wiki registry entry; citations whose doc resolves to a wiki page open in-reader. Unset/empty ⇒ the Ask tab returns a clean "No search collection connected for this wiki" error. E.g. `["wiki", "wiki-life"]`. |
