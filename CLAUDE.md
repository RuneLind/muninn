...
| `CLAUDE_USAGE_URL` | No | `http://127.0.0.1:8787` | Base URL of the **claude-usage** pipeline-ledger service (launchd, port 8787 on the mini), read **server-side** by `GET /api/claude-usage/overview` for the `/models` **Pipeline ledger** card. Muninn reads only `GET /api/pipeline?days=N` (clamped 1–90, default 14) over HTTP with a **10 s** timeout — never claude-usage's sqlite file (schema drift + a 145 MB synchronous `bun:sqlite` read on the event loop); the timeout is sized on the 90-day worst case (802 KB / 1.4 s measured), not the common one. The browser never reaches port 8787 (tailnet viewers + mixed content under `tailscale serve`), so the card renders no external link. Unreachable/non-200/timeout ⇒ 200 + `errors[]` + an honest "claude-usage unreachable" note, never a 5xx; setting this var explicitly is also what keeps the card VISIBLE on a host where the service is down — left unset and unreachable, the card hides. |
