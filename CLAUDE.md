...
| `MUNINN_AGENT_CWD` | No | `~/.muninn/agent-cwd` | Root for the cwds muninn's own `claude -p` spawns use (`src/ai/agent-cwd.ts`, one subdir per bot). A cwd inside the repo makes the CLI load muninn's CLAUDE.md + `.claude/` surface into every watcher gate call (~18k prompt tokens, measured) and file the transcript into the developer's own project folder. Applies only to `spawnHaiku` calls with no explicit cwd — the email watcher (Gmail MCP) and the memory/goal/schedule extractors still pass `bots/<name>/`. |
