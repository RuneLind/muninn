...
| `wikiDir` | string | — | Path to the bot's knowledge wiki (relative to `bots/<name>/`, same semantics as `.mcp.json` paths; resolved to absolute at discovery). Makes the bot's wiki browsable at `/wiki?bot=<name>`. Unset ⇒ the bot has no browsable wiki. Bare `/wiki` still defaults to jarvis (or the `WIKI_DIR` env override). |
