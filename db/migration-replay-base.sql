-- ============================================================================
-- Migration-replay base — FROZEN SNAPSHOT. DO NOT EDIT.
-- ============================================================================
-- This is the exact contents of db/init.sql as it stood immediately BEFORE
-- migration 006-add-bot-name.sql was introduced (git b98c71c, the parent of
-- 070b309 "Add bot-scoped database isolation").
--
-- Why it exists: the original supabase migrations 00001-00005 were consolidated
-- into init.sql and deleted, so db/migrations/ starts at 006. Those migrations
-- ALTER pre-existing tables (e.g. 006 does `ALTER TABLE messages ADD COLUMN
-- bot_name`), so they cannot be replayed against a truly empty database — they
-- need this pre-006 base to apply on top of.
--
-- Used ONLY by src/db/schema-drift.test.ts, which replays migrations 006+ on
-- top of this base and diffs the result against db/init.sql to catch schema
-- drift between the two deploy paths (fresh = init.sql; upgraded = migrations).
--
-- This file is history. It must NOT track current schema changes — new schema
-- changes go in db/init.sql + a new db/migrations/NNN file, never here.
-- ============================================================================
