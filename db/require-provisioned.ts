...
const UNPROVISIONED_INSTRUCTION = [
  ...
  "  the image carries bun and db/, so it can run in place —",
  "",
  "    docker compose exec app bun db/migrate.ts --baseline",
  "    kubectl exec deploy/<app> -- bun db/migrate.ts --baseline",
  "",
...
const UNBASELINED_INSTRUCTION = [
  ...
  "  image ships bun and db/, so it runs in place — no psql, no checkout needed:",
  "",
