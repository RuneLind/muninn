...
const UNPROVISIONED_INSTRUCTION = [
  ...
  "",
  "  The first line needs psql, which this image does NOT ship — run it from a",
  "  machine that has it and can reach this database. The second line does not:",
  "  the image carries bun and db/, so it runs from the image itself. But NOT via",
  "  `exec`: this container exits on the refusal you are reading, so there is no",
  "  running process to exec into. Use a one-off container instead —",
  "",
...
const UNBASELINED_INSTRUCTION = [
  ...
  "",
  "  Record the shipped migrations as applied, against this DATABASE_URL. This",
  ...
  "",
  "  · from a shell that already has this DATABASE_URL and a checkout:",
  "",
